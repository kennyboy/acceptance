 ...
 */
public class SicariusWithKatTest extends Test{
    ...
    public String getShortDescription() {
        return "Sicarius killing Kat with Grim Reaper at disc";
    }
    ...
    public void run(GameState gameState, MoveMaker move) throws AssertionError,
        ...
        */
        ...
        SicariusActivator ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);
        //8  lives left
        ca.complete();
        ...
        ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);        //7
        ca.complete();
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        assert (gameState.getActionDice().length == 1);
        ca = (SicariusActivator) move.chooseCardToActivate(1);

        ca.chooseDiceDisc(1);        //6
        ca.complete();
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        ...
        ca.chooseDiceDisc(1);        //5
        ca.complete();
        assert (gameState.getActionDice().length == 2);
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);        //4
        ca.complete();
        assert (gameState.getActionDice().length == 1);
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        gameState.setActionDice(new int [] {1, 1, 1});
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);        //3
        ca.complete();
        assert (gameState.getActionDice().length == 2);
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);        //2
        ca.complete();
        assert (gameState.getActionDice().length == 1);
        assert (gameState.getPlayerCardsOnDiscs(PLAYER_2)[0].toString().equals(Card.KAT.toString()));
        ca = (SicariusActivator) move.chooseCardToActivate(1);
        ca.chooseDiceDisc(1);        //1
        ca.complete();
        assert (gameState.getActionDice().length == 0);
